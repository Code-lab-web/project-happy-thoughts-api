...
function styleText(
  format,
  text,
  { validateStream = true, stream = process.stdout } = {}
) {
...
function getCallSites(frameCount = 10, options) {
  ...
  if (
    ...
    (getOptionValue("--enable-source-maps") && options.sourceMap !== false)
  ) {
